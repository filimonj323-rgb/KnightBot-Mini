 ...
 */
// Inafunga sockets zote za wateja wa pairing haraka wakati process
// inapopokea SIGTERM/SIGINT (Railway ikianza deploy mpya) — bila hii,
// container ya zamani inaendelea kushikilia connection za WhatsApp za kila
// mteja mpaka OS imalize process kwa nguvu, ambayo inaweza kuchelewa kidogo
// na kusababisha "mgongano wa connection (statusCode 440)" na container
// mpya inayoanza wakati huo huo. Kuifunga mapema hapunguza muda wa overlap.
async function shutdownAllInstances() {
  const phoneNumbers = Array.from(instances.keys());
  for (const phoneNumber of phoneNumbers) {
    const record = instances.get(phoneNumber);
    try {
      record?.sock?.ws?.close?.();
      record?.sock?.end?.(new Error('shutdown'));
    } catch (e) {
      // Silent — tunajaribu tu kufunga vizuri, si lazima ifanikiwe 100%
    }
  }
  console.log(`[pairing] shutdownAllInstances: ${phoneNumbers.length} sockets zimefungwa kabla ya kuzima process.`);
}
