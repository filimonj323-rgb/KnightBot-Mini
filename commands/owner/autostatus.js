 ...
 * Imebadilishwa kufuata muundo wa MR.IT MEDIATOR (JSON database)
 */

const fs = require('fs');
const path = require('path');

// Path ya autostatus settings file (ndani ya database folder ya bot)
...
function load() {
  try {
    if (!fs.existsSync(AUTOSTATUS_DB)) {
      fs.writeFileSync(AUTOSTATUS_DB, JSON.stringify(DEFAULTS, null, 2));
      return { ...DEFAULTS };
    }
    return { ...DEFAULTS, ...JSON.parse(fs.readFileSync(AUTOSTATUS_DB, 'utf-8')) };
  } catch (e) {
    return { ...DEFAULTS };
  }
}
...
function save(data) {
  try {
    const dir = path.dirname(AUTOSTATUS_DB);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(AUTOSTATUS_DB, JSON.stringify(data, null, 2));
  } catch (e) {
